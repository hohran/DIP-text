...
\begin{itemize}
  \item $A = (A_1, \ldots, A_n)$ is a list of \emph{argument type sets}, with $A_i \subseteq \Types$ being the set of types acceptable at position $i$,
  \item $\phi : \Vals^n \to \Vals$ maps concrete argument values to a result value.
\end{itemize}
We denote the set of all functions with $\Funs$ and write $\sigma_F : \Terms \to \Funs$ for the mapping from function name terms to their definitions.
...
\begin{itemize}
  ...
  \item a \emph{declaration}: an introduction of locally scoped variables (w.r.t. the current rule body), in one of the following forms:
    \begin{itemize}
      ...
      \item \emph{existential declaration} \texttt{some} [$k$,] $x$ [\texttt{in} $t$]:
            ...
            With \texttt{in}, followed by a collection $t \in \Terms$, $x$ quantifies the elements of $\sigma(t)$ and (optionally) $k$ quantifies its keys (w.r.t. $x$).
      ...
      \item \emph{universal declaration} \texttt{every} [$k$,] $x$ \texttt{in} $t$ \texttt{\{} $e_1 \cdots e_n$ \texttt{\}}:
            must be bound to a collection $\sigma(t)$, with $x$ quantifying its elements and (optionally) $k$ quantifying its keys (w.r.t. $x$).
...
\begin{itemize}
  \item $B_v \in \Terms$ is the \emph{value} the body contributes to the head; when absent from the source, $B_v = \true$,
  \item $B_E = (e_1, \ldots, e_m)$ is an ordered list of expressions forming the body condition.
\end{itemize}

A body $B$ is \emph{satisfied}, written $\Holds(B)$, iff every expression in $B_E$ holds:
$$\Holds(B) \iff \forall_{e \in B_E} : \Holds(e)$$
When $B_E$ is empty, $B$ is satisfied unconditionally.
...
$$\Holds(B_i) \text{ is considered only if } \forall_{j < i} : \neg\Holds(B_j)$$
...
\begin{example}
  ...
  $\RuleKey(h) = \texttt{action}$, contributing one element per satisfied binding.
  The rule on line~\ref{ex:has_role} is a function definition with
  $\RuleArgs(h) = (\texttt{user}, \texttt{role})$.
  The rule on line~\ref{ex:allow} is a complete definition containing two bodies (separated by \texttt{else}); if both bodies would not be satisfied, the variable specified in the head -- \texttt{allow} -- would be assigned the fallback value \false as defined on line~\ref{ex:default}.
\end{example}
...
User-defined functions enter $\sigma_F$ (as defined is Section~\ref{ss:functions}) through their rules: for a function name $f$
defined by rules, $\sigma_F(f)$ is constructed from the argument patterns $\RuleArgs(h)$
...
allow := false if { input.role == "editor" }
\end{lstlisting}
  When \texttt{input.role} is \texttt{"editor"}, both bodies are satisfied with
  differing values $\true$ and $\false$, so $v(h_1, \mathbf{B}_1) \neq v(h_2, \mathbf{B}_2)$
  and neither is $\Undef$ — a conflict.
  Replacing the second rule with \texttt{allow := true} resolves the conflict, since
  both rules now agree on the contributed value.
\end{example}
