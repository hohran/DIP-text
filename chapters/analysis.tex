...
A rule body $B$ in a policy $\Pkg$ is said to be \emph{subsumed} when omitting $B$ from $\Pkg$ does not change its semantics, i.e., $B$ is irrelevant to the result of $\Pkg$ in any evaluation context.
In a programming environment, this corresponds to the notion of \emph{dead code}.
Detecting subsumed bodies aids the understanding of a given policy and allows its encoding to be made more compact.

This section presents an analysis for the specified fragment of \rego policies, identifying all subsumed bodies of a given head variable.
...
For a head variable $x \in \Globals$, we denote by $\Rules_x \subseteq \Rules$ the set of all rules assigning to $x$, i.e., $\Rules_x = \{(h,\Bodies) \in \Rules \mid \RuleRef(h) = x\}$.

A body $B$ of a rule $(h,\Bodies) \in \Rules_x$ is \emph{subsumed} iff the following holds:
\begin{align*}
  \forall C \in S{:}\;
  \Bigg(\bigvee_{\substack{(h',\Bodies') \in \Rules_x}}\;\bigvee_{\substack{B' \in \Bodies' \\ B' \neq B}} \Applies(B')\Bigg) \implies \Applies(B)
\end{align*}
That is, in every evaluation context in which any other body assigning to $x$ applies, $B$ also applies.
Under this condition, removing $B$ from $\Pkg$ cannot alter the value of $x$ in any context: whenever $B$ would have contributed a value, some other body of $\Rules_x$ contributes the same value (since $\Pkg$ is assumed collision-free), and so the result is unchanged.

\textit{The definition assumes that no collisions are possible in $\Pkg$, as defined in Section~\ref{s:collision}.}

\subsection{The Approach}

The task of the analysis is to identify all subsumed bodies among the rules of a given head variable $h_{sub} \in \Globals$.
As in the collision analysis, we generalize the possible evaluation contexts by a provided $\Schema$, and the rules outside of $\Rules_{h_{sub}}$ are accounted for by the \emph{partial encoding} $E$ over $h_{sub}$, encoding all rules from $\Rules \setminus \Rules_{h_{sub}}$.

For a rule $r_i \in \Rules_{h_{sub}}$ and a body $B \in r_i$, we define the disjunction of apply-propositions of all bodies belonging to rules of $h_{sub}$ \emph{other than} $r_i$:
$$\Rest(r_i) \;\mathrel{:=}\; \bigvee_{\substack{r_j \in \Rules_{h_{sub}} \\ j \neq i}} \;\bigvee_{B' \in r_j} \Applies(B')$$
The body $B$ is subsumed, given the symbolic evaluation context, iff
$$E \;\land\; \Applies(B) \;\land\; \neg\Rest(r_i)$$
is unsatisfiable.
Intuitively, there exists no model in which $B$ applies and at the same time none of the bodies from the remaining rules of $h_{sub}$ apply; equivalently, $\Applies(B)$ is implied by $\Rest(r_i)$ extended with whatever constraints the rest of the policy imposes.

Note that bodies of an else-chain within a single rule $r_i$ are mutually exclusive by construction: the proposition $\Applies(B_k)$ already encodes that none of the preceding bodies $B_1,\ldots,B_{k-1}$ of $r_i$ held.
Consequently, no body can be subsumed by a sibling within the same rule, and the check is restricted to bodies of \emph{other} rules of $\Rules_{h_{sub}}$.

\begin{figure}
  \caption{Example of a \rego policy containing a subsumed body.}
  \label{fig:subsu:ex}
  \begin{lstlisting}[language=Rego]
allow if input.role == "admin"
allow if { input.role == "admin"; input.active == true }
\end{lstlisting}
\end{figure}

\begin{example}
  Consider the policy given in Figure~\ref{fig:subsu:ex}, with the head variable $h_{sub} = \texttt{allow}$.
  Let $B_1$ denote the body of the first rule and $B_2$ the body of the second rule, where both assign the value \texttt{true}.

  Since the policy contains no rules other than those assigning to \texttt{allow}, the partial encoding is $E = \true$.

  We check whether $B_2$ is subsumed.
  The set $\Rest$ for the rule of $B_2$ consists of $\Applies(B_1)$ only, and so the subsumption condition becomes:
  $$\Applies(B_2) \land \neg\Applies(B_1) \;\equiv\; (r = \texttt{"admin"} \land a = \true) \land \neg(r = \texttt{"admin"})$$
  where $r$ denotes \texttt{input.role} and $a$ denotes \texttt{input.active}.
  This formula is unsatisfiable, and thus $B_2$ is subsumed: whenever its condition holds, $B_1$ also applies and produces the same value, so omitting $B_2$ does not affect the policy.

  Conversely, checking $B_1$ yields
  $$\Applies(B_1) \land \neg\Applies(B_2) \;\equiv\; r = \texttt{"admin"} \land \neg(r = \texttt{"admin"} \land a = \true),$$
  which is satisfied by, e.g., \texttt{input.role} $=$ \texttt{"admin"} and \texttt{input.active} $=$ \texttt{false}.
  Hence $B_1$ is not subsumed.
\end{example}
