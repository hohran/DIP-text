...
Three judgment forms are used throughout the inference:
\begin{align*}
...
\inferrule{
  ...
  \Sigma \vdashT \texttt{data.}\pkg.x : \ty
}
\]
...
\inferrule{
  x[s_1]\cdots[s_k] \\
  % x\cdots[s_i] \in \dom(\typof) \\
  \Sigma \vdashT x\cdots[s_i] : \Array{\ty}^* \\
  i \in 0..(k-1)
}{
  \Sigma \vdashT x\cdots[s_{i+1}] : \ty \cup \Undef
}
\qquad
\textbf{(T-SetM)}\quad
\inferrule{
  x[s_1]\cdots[s_k] \\
  \Sigma \vdashT x\cdots[s_i] : \RegoSet{\ty_s}^* \\
  \Sigma \vdashT s_{i+1} : \ty \\
  i \in 0..(k-1)
}{
  \Sigma \vdashT x\cdots[s_{i+1}] : \ty \cup \Undef
}
...
\inferrule{
  r = x[s_1]\cdots[s_k] \\
  \Sigma \vdashT x\cdots[s_i] : \Obj{s_{i+1}{:} \ty, \ldots} \\
  i \in 0..(k-1)
}{
  \Sigma \vdashT x\cdots[s_{i+1}] : \ty
}
\]
...
\inferrule{
  r = x[s_1]\cdots[s_k] \\
  \Sigma \vdashT x\cdots[s_i] : \Obj{v_1{:}\ty_1,\ldots,v_n{:}\ty_n} \\
  s_{i+1} \notin \bigcup_{i \in 1..n}\{v_i\} \\
  s_{i+1} \notin \Vars
}{
  \Sigma \vdashT x\cdots[s_{i+1}] : \Undef
}
\]
...
For brevity, the rules where object types appear in unions were not included.
