...
% % This chapter will be heavily reliant on section \todo{ref} defining the type system of Rego.
%
% \section{Type System of Rego}
%
% The set of \emph{Rego types} $\mathbb{T}$ is defined by the grammar:
% \begin{align*}
% \ty \;::=\; & \Str \mid \Int \mid \Bool \mid \Null \mid \RegoSet \mid \Unknown \\
%    \mid\;   & \Array{\ty} \\
...
% \begin{itemize}
%   \item \emph{Atomic types} $\Str, \Int, \Bool, \Null, \RegoSet$ correspond to JSON primitives and Rego sets.
%   \item $\Array{\ty}$ is a possibly \emph{heterogeneous} array whose elements all have type $\ty$.
%   \item $\Obj{f_1{:}\ty_1,\ldots,f_n{:}\ty_n}$ is an \emph{object type} with fields $f_i$ typed $\ty_i$.
%     Object types may additionally allow \emph{additional properties}, recorded by a Boolean flag; we omit this detail in the grammar for brevity.
%   \item $\ty_1 \cup \cdots \cup \ty_m$ is a \emph{union} type, modelling values that may inhabit any of the member types.
%   \item $\Unknown$ is a \emph{placeholder} type used when no information is available yet; it is the least element of the precision order.
% \end{itemize}
%
% \section{Type Lattice}
% \label{sec:lattice}
% %%=============================================================
%
% \subsection{Precision Order}
%
% \begin{definition}[Precision order $\leqT$]
% \label{def:precision}
% The precision order $\leqT$ on $\mathbb{T}$ is the smallest preorder satisfying:
% \begin{enumerate}
