...
\section{Type Analysis Algorithm}
...
type via $\refine{}{}$, or extends a union with a new member via $\unionize{}{}$
Since the set of terms in $\Rules$ and $\dom(\Schema)$ is finite, and the
type lattice $(\Types, \preq)$ has bounded height, the sequence $(\typof_k)_{k \geq 0}$
is eventually constant and thus converges to the fixpoint of $\typof$.
